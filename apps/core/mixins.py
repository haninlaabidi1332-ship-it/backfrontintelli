# apps/core/mixins.py
"""
Mixins complets pour IntelliOLT – AI-Powered Fiber Network Supervision Platform
...
- Métriques et performances (optionnel)
"""
import logging
import json
...
class AuditMixin(models.Model):
    ...
    - Journalisation automatique (optionnelle)
    """
    ...
    def log_action(self, action, request=None, details=None):
        ...
        Nécessite le modèle UserActivity dans apps.users.
        Si non disponible, ignore silencieusement.
        """
        try:
...
class SoftDeleteMixin(models.Model):
    ...
    active_objects = models.Manager()  # alias, mais à utiliser avec filter(is_deleted=False)
...
class CacheableMixin(models.Model):
    ...
    def invalidate_cache(self):
        ...
        # Simple invalidation par pattern; nécessite django-redis-cache ou similaire
        try:
            cache.delete_pattern(f"{self.cache_prefix}{self.__class__.__name__.lower()}:*")
        except AttributeError:
            # Si delete_pattern n'existe pas (cache mémoire), on supprime une clé générique
            cache.delete(f"{self.cache_prefix}{self.__class__.__name__.lower()}:{self.pk}")
...
class AuditLogMixin:
    ...
    def perform_destroy(self, instance):
        ...
        if hasattr(instance, 'delete'):
            # Appel à delete() qui gère le soft delete si présent
...
class OwnerFilterMixin:
    ...
    - Admin ou Superviseur voit tout
    - Les autres voient leurs propres données
    """
    ...
    def get_queryset(self):
        ...
        # Dans IntelliOLT, admin et supervisor ont accès à tout
...
class SoftDeleteViewMixin:
    ...
    def restore(self, request, pk=None):
        """Restaure un élément supprimé logiquement"""
        # Utilisation du manager all_objects pour inclure les soft-deleted
        ...
        if not instance:
            return Response(
                {"status": False, "message": "Élément non trouvé"},
                status=status.HTTP_404_NOT_FOUND
            )
        
        if not getattr(instance, 'is_deleted', False):
            return Response(
                {"status": False, "message": "Cet élément n'est pas supprimé"},
        ...
        return Response(
            {"status": True, "message": "Élément restauré avec succès", "data": serializer.data},
...
class BulkActionViewMixin:
    ...
    def bulk_delete(self, request):
        ...
        if not ids:
            return Response(
                {"status": False, "message": "Aucun ID fourni"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if count == 0:
            return Response(
                {"status": False, "message": "Aucun élément trouvé"},
                status=status.HTTP_404_NOT_FOUND
            )
        ...
        return Response(
            {"status": True, "message": f"{count} élément(s) supprimé(s)", "deleted_count": count},
            status=status.HTTP_200_OK
        )
    ...
    def bulk_update(self, request):
        ...
        if not ids or not fields:
            return Response(
                {"status": False, "message": "ids et fields sont requis"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not safe_fields:
            return Response(
                {"status": False, "message": "Aucun champ valide à mettre à jour"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"status": True, "message": f"{count} élément(s) mis à jour", "updated_count": count},
            status=status.HTTP_200_OK
        )
    ...
    def bulk_status(self, request):
        ...
        if not ids or not status_value:
            return Response(
                {"status": False, "message": "ids et status sont requis"},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class CachedQueryMixin:
    ...
    def invalidate_cache(self):
        ...
        try:
            ...
            # Cache simple, on ne peut pas supprimer par pattern facilement
            pass


class ExportMixin:
    """
    Mixin pour l'export de données via pandas
    Nécessite pandas et openpyxl installés.
    """
    ...
    def export(self, request):
        ...
        if export_format not in self.export_formats:
            return Response(
                {"status": False, "message": f"Format non supporté. Choisir parmi: {', '.join(self.export_formats)}"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not data:
            return Response(
                {"status": False, "message": "Aucune donnée à exporter"},
                status=status.HTTP_404_NOT_FOUND
            )
        ...
        try:
            ...
            return Response(
                {"status": False, "message": f"Bibliothèque manquante: {str(e)}. Installez pandas et openpyxl."},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
        except Exception as e:
            logger.exception("Export error")
            return Response(
                {"status": False, "message": f"Erreur d'export: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class StatisticsMixin:
    ...
    def statistics(self, request):
        ...
        return Response(
            {"status": True, "message": "Statistiques récupérées", "data": stats},
            status=status.HTTP_200_OK
        )
...
class PermissionMixin:
    """
    Mixin pour la gestion des permissions (DRF)
    """
    permission_map = {}
    model_name = None  # À définir dans la vue
...
# ============================================================================
# MIXINS DE PERFORMANCE (optionnels)
# ============================================================================
...
class ValidationMixin:
    ...
    def validate_required_fields(self, data, required_fields):
        ...
        if missing:
            from rest_framework.exceptions import ValidationError
            raise ValidationError({field: 'Ce champ est requis' for field in missing})
...
class TestMixin:
    """
    Mixin pour les tests unitaires (IntelliOLT)
    """
    ...
    def create_test_user(self, **kwargs):
        ...
        defaults = {
            ...
            'role': 'operator',   # rôle par défaut pour IntelliOLT
        }
