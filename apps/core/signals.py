...
"""
Signaux Django pour l'application core IntelliOLT – AI-Powered Fiber Network Supervision Platform
"""
import logging
...
def config_post_save(sender, instance, created, **kwargs):
    ...
    logger.info(f"🔧 Configuration '{instance.key}' {action} (IntelliOLT)")
    ...
    try:
        ...
        # Cache simple (locmem) ne supporte pas delete_pattern
        pass
    ...
    if instance.pk:
        try:
            ...
            if old_instance.value != instance.value:
                logger.debug(f"📝 Configuration '{instance.key}' va être modifiée (IntelliOLT)")
...
def config_post_delete(sender, instance, **kwargs):
    ...
    """
    logger.info(f"🗑️ Configuration '{instance.key}' supprimée (IntelliOLT)")
...
def invalidate_config_cache(sender, **kwargs):
    ...
    try:
        ...
        pass
    logger.debug("🧹 Cache des configurations invalidé (IntelliOLT)")
...
def connect_signals():
    ...
    """
    logger.info("🔌 Signaux core IntelliOLT connectés")
