...
"""
Modèles Utilisateurs, Rôles et Permissions pour IntelliOLT – AI-Powered Fiber Network Supervision Platform
"""
...
class UserManager(BaseUserManager):
    """Gestionnaire personnalisé pour Utilisateur IntelliOLT"""
...
class User(AbstractUser, BaseModel):
    """
    Modèle utilisateur étendu pour IntelliOLT – Supervision OLT avec IA
    """
    
    # Rôles IntelliOLT (administration hiérarchique)
    ROLE_CHOICES = [
    ...
    # Preferences
    timezone = models.CharField('Fuseau horaire', max_length=50, default='Africa/Tunis')
    ...
    # ========================================================================
    # PROPRIÉTÉS DE RÔLE POUR INTELLIOLT
    # ========================================================================
    ...
    @property
    def can_view_olt(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    @property
    def can_manage_olt(self):
        return self.is_admin
    
    @property
    def can_view_ont(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    @property
    def can_manage_ont(self):
        return self.role in ['admin', 'supervisor']
    
    # --- SNMP ---
    @property
    def can_view_snmp_metrics(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    @property
    def can_manage_snmp_oids(self):
        return self.role in ['admin', 'supervisor']
    
    # --- BFD ---
    @property
    def can_view_bfd_sessions(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    @property
    def can_manage_bfd_sessions(self):
        return self.is_admin
    
    # --- Alertes ---
    @property
    def can_view_alert_rules(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    @property
    def can_manage_alert_rules(self):
        return self.role in ['admin', 'supervisor']
    ...
    @property
    def can_acknowledge_alert(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    @property
    def can_resolve_alert(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    # --- IA / Machine Learning ---
    @property
    def can_view_anomalies(self):
        if self.is_admin: return True
        return self.role in ['supervisor', 'operator']
    
    @property
    def can_manage_ml_models(self):
        return self.is_admin
...
class Team(BaseModel):
    """Équipes pour organiser les utilisateurs IntelliOLT (ex: équipe NOC, équipe terrain)"""
...
class Permission(BaseModel):
    """Permissions personnalisées pour IntelliOLT"""
...
class Role(BaseModel):
    """Rôles avec permissions associées pour IntelliOLT"""
    name = models.CharField('Nom', max_length=100, unique=True)
    description = models.TextField('Description', blank=True)
...
class UserActivity(BaseModel):
    """Journal des activités utilisateur pour IntelliOLT"""
    ...
    ACTION_CHOICES = [
        ...
        ('export', '📤 Export'),
        ('import', '📥 Import'),
        ('snmp_poll', '📊 Collecte SNMP'),
        ('bfd_check', '🔗 Vérification BFD'),
        ('acknowledge_alert', '✅ Acquittement alerte'),
        ('resolve_alert', '✔️ Résolution alerte'),
        ('anomaly_detection', '🧠 Détection anomalie IA'),
        ('ml_model_train', '🤖 Entraînement modèle ML'),
        ('lab_action', '🧪 Action EVE-NG'),
        ('schedule_report', '📅 Planification rapport'),
    ]
