...
def aggregate_kpi(period='hour'):
    ...
    avg_mem  = _avg_metric('mem')
    avg_temp = _avg_metric('temp')
    avg_rx   = _avg_metric('rx')
    ...
    KPIHistory.objects.create(
        ...
        # total_onts / online_onts kept at 0 — no ONTs in this topology
        total_onts=0, online_onts=0,
    ...
    logger.info(f"KPIs agrégés ({period}) — {active_olts}/{total_olts} OLTs actifs, BFD {bfd_up}/{bfd_total}")
...
def _generate_pdf(kpis, report):
    ...
    story.append(Paragraph(
        f"FTTH Network Supervision &mdash; {report.get_report_type_display()}",
        s_subtitle,
    ))
    ...
    generated_by = (
        ...
        if report.generated_by else 'System'
    )
    meta_rows = [
        ['Report',        report.name],
        ['Period',        f"{report.date_from.strftime('%d/%m/%Y')} - {report.date_to.strftime('%d/%m/%Y')}"],
        ['Generated on',  now.strftime('%d/%m/%Y at %H:%M')],
    ...
    summary_rows = [
        ...
        ['Active OLTs / Total',    f"{active_olts} / {total_olts}",
            _status_cell(olt_pct >= 100)],
        ['Average CPU',
            f"{avg_cpu} %" if avg_cpu is not None else ', (no data)',
        ...
        ['Active Alerts',          str(active_alerts),
            _status_cell(active_alerts == 0, '✓ None', f'⚠ {active_alerts} active')],
        ['Anomalies in Period',    str(period_anomalies),
            _status_cell(period_anomalies < 3, '✓ Nominal', f'⚠ {period_anomalies} detected')],
    ]
    ...
    # ── OLT STATUS ────────────────────────────────────────────────────────────
    story.append(Paragraph("2. OLT Equipment Status", s_h2))
    olt_hdr = ['Hostname', 'Site', 'IP Address', 'Vendor', 'Model', 'Status']
    olt_rows = [olt_hdr]
    STATUS_LABEL = {'active': 'ACTIVE', 'maintenance': 'MAINTENANCE', 'inactive': 'INACTIVE'}
    ...
    # ── KPI JOURNALIERS ───────────────────────────────────────────────────────
    story.append(Paragraph("3. Daily KPIs", s_h2))
    if kpis_list:
        kpi_hdr = ['Date', 'Active OLTs', 'Avg CPU', 'Avg Mem.', 'SNMP %', 'BFD UP', 'Anomalies', 'Alerts']
        kpi_rows = [kpi_hdr]
        for k in kpis_list:
            kpi_rows.append([
        ...
        kpi_t = Table(kpi_rows, colWidths=[2.8*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2.3*cm, 2*cm, 2.3*cm, 2*cm])
        kpi_t.setStyle(TableStyle(_hdr_style()))
        story.append(kpi_t)
    else:
        story.append(Paragraph(
            "No daily KPI data available for this period.",
            s_note,
        ))
    story.append(Spacer(1, 16))

    # ── ALERTS ────────────────────────────────────────────────────────────────
    story.append(Paragraph("4. Alerts for the Period", s_h2))
    alerts_qs = Alert.objects.filter(
        first_seen__range=(report.date_from, report.date_to)
    ).select_related('olt').order_by('-first_seen')[:20]

    if alerts_qs.exists():
        alert_hdr = ['Date', 'OLT', 'Severity', 'Status', 'Message']
    ...
    story.append(Paragraph(
        "Confidential document, internal use SOTETEL ISP. "
        "Automatically generated by IntelliOLT v2.0.",
        s_small,
    ))
...
def _generate_excel(kpis, report):
    ...
    ws_sum['A3'] = (
        f"Period: {report.date_from.strftime('%d/%m/%Y')} - "
        f"{report.date_to.strftime('%d/%m/%Y')}"
    )
    ws_sum['A3'].font = FONT_SUB
    ws_sum['A4'] = f"Generated on: {now.strftime('%d/%m/%Y at %H:%M')}"
    ...
    metric_rows = [
        ('Indicator', 'Value', 'Detail', 'Status'),
        ('Active OLTs', f"{active_olts}/{total_olts}", '', '✓ OK' if active_olts == total_olts else '⚠ Check'),
        ('Avg CPU', f"{avg_cpu} %" if avg_cpu else ',', 'over period', '✓ OK' if not avg_cpu or avg_cpu < 70 else '⚠'),
        ('Avg Memory', f"{avg_mem} %" if avg_mem else ',', 'over period', '✓ OK' if not avg_mem or avg_mem < 80 else '⚠'),
        ('SNMP Rate', f"{avg_snmp} %" if avg_snmp else ',', 'polling success', '✓ OK' if not avg_snmp or avg_snmp > 95 else '⚠'),
        ('Active Alerts', active_alerts, '', '✓ None' if active_alerts == 0 else f'⚠ {active_alerts}'),
        ('Period Anomalies', period_anomalies, '', '✓ OK' if period_anomalies < 3 else '⚠'),
    ]
    ...
    # ════════════════════════════════════════════════════════════════════════
    ws_kpi = wb.create_sheet("Daily KPIs")
    ws_kpi.sheet_view.showGridLines = False
    kpi_headers = [
        'Date', 'Total OLTs', 'Active OLTs', 'Avg CPU (%)', 'Avg Mem. (%)',
        'Temp. (°C)', 'RX (dBm)', 'SNMP %', 'BFD UP', 'Anomalies', 'Alerts',
    ]
    ...
    if not kpis_list:
        ws_kpi.cell(row=2, column=1, value='No data available for this period').font = FONT_BOLD
    _auto_width(ws_kpi)
    ...
    olt_headers = ['Hostname', 'Site', 'IP Address', 'Vendor', 'Model', 'Status']
    ...
    alert_headers = ['Date', 'OLT', 'Severity', 'Status', 'Message']
    ...
    if not alerts_qs.exists():
        ws_alert.cell(row=2, column=1, value='No alerts for this period').font = FONT_BOLD
